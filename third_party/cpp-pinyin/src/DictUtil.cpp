...
#include "cpp-pinyin/G2pglobal.h"  // getDictData（内存优先）
#include "cpp-pinyin/U16Str.h"

namespace Pinyin
{
    // 打开 dict 数据流：内存优先（按 langdir/filename 查注入数据），未命中则 fallback 文件
    static std::shared_ptr<std::istream> openStream(const std::filesystem::path &dict_dir) {
        const std::string key = dict_dir.parent_path().filename().string() + "/" +
                                dict_dir.filename().string();
        if (const std::string *data = getDictData(key)) {
            return std::make_shared<std::istringstream>(*data);
        }
#ifdef _WIN32
        const std::wstring wdict_dir = dict_dir.wstring();
        return std::make_shared<std::ifstream>(wdict_dir.c_str());
#else
        return std::make_shared<std::ifstream>(dict_dir.c_str());
#endif
    }
    ...
    static bool processFile(std::istream &file, std::unordered_map<K, V> &resultMap,
                            const char &sep1, KeyFunc keyProcessor, ValueFunc valueProcessor) {
        if (!file) {
            std::cerr << "Error: Unable to open dict stream" << std::endl;
            return false;
        }
    ...
    bool loadDict(const std::filesystem::path &dict_dir,
        ...
        auto stream = openStream(dict_dir);
        return processFile(*stream, resultMap, sep1,
